...
const char epilogue[] = "H\xc7\xc0<\x00\x00\x00\x0f\x05"; // mov rax, 0x3c ; syscall
...
{
    UPDATE_REGS();
    printf("rax: 0x%lx, rdi: 0x%lx, rsi: 0x%lx, rdx: 0x%lx, r8: 0x%lx\n",regs.rax, regs.rdi, regs.rsi, regs.rdx, regs.r8);
    write(1, "Disallow reg!!\n", 15);
}
...
int main()
{
    ...
    // mmap(start_addr, length, protection)
    stack = (char *) mmap((void *) 0x30000, 0x8000, PROT_READ | PROT_WRITE, MAP_SHARED | MAP_ANONYMOUS, -1, 0) + 0x4000;
    new_code_buf = (char *) mmap((void *) 0x40000, 0x1000, PROT_READ | PROT_WRITE | PROT_EXEC, MAP_SHARED | MAP_ANONYMOUS, -1, 0);
